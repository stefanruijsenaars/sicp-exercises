...
(define squarew (lambda (x) (let ((i (car x))
                                                              (j (cadr x)))
                                                          (+ (* i i) (* j j)))))

(define a (weighted-pairs integers integers squarew))
...
(define solution
  (stream-filter
   ...
   (stream-map
    (lambda (x y z)
      (if (= (squarew x) (squarew y) (squarew z))
          (begin
            (display x) (newline)
            (display y) (newline)
            (display z) (newline)
...
(stream-ref solution 4)
(stream-ref solution 5)