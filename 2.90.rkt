...
(define (install-dense-package)
  ...
  ; get term with largest exponent
  (define (first-term term-list) (make-term (- (length term-list) 1) (car term-list)))
  ; order must be larger than current length
  (define (adjoin-term term term-list)
    (define (adjoin-iter term-list term)
          (cond ((= (order term) (- (length term-list) 1)) term-list)
                ((= (order term) (length term-list)) (adjoin-iter (cons (coeff term) term-list) term))
                (else (adjoin-iter (cons 0 term-list) term))))
    (if (=zero? (coeff term))
        term-list
        (adjoin-iter term-list term)))
...
(define (install-sparse-package)
  ...
  ; order must be larger than largest order
  (define (adjoin-term term term-list)
    (if (=zero? (coeff term))
        term-list
        (cons term term-list)))
...
(display (adjoin-term (make-term 0 3) (the-empty-termlist 'sparse)))
(newline)
(display (adjoin-term (make-term 2 3) (the-empty-termlist 'dense)))